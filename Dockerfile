...
RUN apt-get update && apt-get install -y software-properties-common \
  ...
  && apt-get update  \
  && apt-get install -y python3.8 python3-pip python3.8-dev libproj-dev \
  ...
  && cd /donegal-bus-routes && python3.8 -m pip install -r requirements.txt
...
RUN python3.8 -m ipykernel install --user && jupyter notebook --generate-config --allow-root
